...
export default function Sidebar({ state, width, height, sidebarComponents }) {
  ...
  let sorter = [
    { index: 0, condition: false, dom: <PanelGuides state={state}/> },
    { index: 1, condition: false, dom: <PanelLayers state={state} /> },
    ...
    { index: 3, condition: false, dom: <PanelGroups mode={state.mode} groups={state.scene.groups} layers={state.scene.layers} /> },
